...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    if( auto* dwarfCh = dynamic_cast<Dwarf*>(character) )
    {
        dwarfCh->boostArmor( item.getBoost() * 1.1 );
    }
    else if( auto* paladinCh = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        paladinCh->boostArmor( item.getBoost() * 1.3 );
    }
    else if( auto* dragonSlayerCh = dynamic_cast<DragonSlayer*>(character))
    {
        dragonSlayerCh->boostArmor( item.getBoost() * 1.5 );
    }
    else if( auto* dragonCh = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    if( auto* dwarfCh = dynamic_cast<Dwarf*>(character) )
    {
        dwarfCh->boostHitPoints(item->getBoost() * 2);
    }
    else if( auto* paladinCh = dynamic_cast<Paladin*>(character) )
    {
        paladinCh->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* dragonSlayerCh = dynamic_cast<DragonSlayer*>(character))
    {
        dragonSlayerCh->boostHitPoints(item->getBoost() * 1.25);
    }
    else if( auto* dragonCh = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    if( auto* dwarfCh  = dynamic_cast<Dwarf*>(character) )
    {
        dwarfCh ->boostAttackDamage(item->getBoost() * 1.5);
    }
    else if( auto* paladinCh = dynamic_cast<Paladin*>(character) )
    {
        paladinCh->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* dragonSlayerCh = dynamic_cast<DragonSlayer*>(character))
    {
        // assert(false);
        ...
        dragonSlayerCh->boostAttackDamage(item->getBoost() * dragonSlayerCh->getAttackDamage());
    }
    else if( auto* DragonCh = dynamic_cast<Dragon*>(character) )
    {
